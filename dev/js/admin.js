...
function renderAdminOverview(){
  ...
  if(!users.length){el.innerHTML='<p class="text-muted">אין פיזיותרפיסטים עדיין.</p>';return;}
...
function renderRulesList(){
  const groups=[
    {cat:'assessment',label:'הערכת מטופל',icon:'📊'},
    {cat:'followup',label:'טיפול שוטף',icon:'🔄'},
    {cat:'general',label:'כללי',icon:'📋'},
    {cat:'non-clinical',label:'פעולות מנהלתיות',icon:'⏱'},
  ];
  const rules=_rules;
  let html='';
  groups.forEach(g=>{
...
function showAddRuleModal(){
  ...
  html+='<div class="form-group"><label>סוג פעילות</label><select id="nr-cat" onchange="toggleRuleFields()"><option value="assessment">הערכת מטופל</option><option value="followup">טיפול שוטף</option><option value="general">כללי</option><option value="non-clinical">פעולות מנהלתיות</option></select></div>';
...
function showEditRuleModal(id){
  ...
  const isNC=r.category==='non-clinical';
  const catLabel={assessment:'הערכת מטופל',followup:'טיפול שוטף',general:'כללי','non-clinical':'פעולות מנהלתיות'}[r.category]||r.category;
...
function showEditUserModal(id){
  ...
  let html=`<div class="modal-title">עריכת משתמש</div>
    ...
    <div class="form-group"><label>תפקיד</label>
      <select id="eu-role" onchange="toggleEuFields()" style="width:100%;padding:10px;border:1.5px solid var(--border);border-radius:10px;font-family:Rubik,sans-serif;font-size:14px;background:var(--surface)">
        <option value="physio">פיזיותרפיסט/ית</option>
  ...
  document.getElementById('eu-role').value=u.role||'physio';
...
async function saveNewUser(){
  ...
  const role=document.getElementById('nu-role')?.value||'physio';
  ...
  try{
    ...
    showToast(role==='admin'?'המנהל/ת נוסף/ה! ✅':'הפיזיותרפיסט/ית נוסף/ה! ✅');
  }catch(e){showToast('שגיאה: '+e.message);}
}
